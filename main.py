...
def CreateComment(content, author_id, reply_to_id):
  ...
  comment = Comment(content=bleach.clean(content), author_id=author_id, reply_to_id=repid)
...
def EditPost(m_post, title, content):
  m_post.title = bleach.clean(title)
  m_post.content = bleach.clean(content)
...
def login():
  ...
  if login_form.submitLogin.data and login_form.validate():
    ...
    if Email.query.filter_by(email=email).first() or User.query.filter_by(username=register_form.username.data).first():
      ...
      user = CreateUser(
        username=bleach.clean(register_form.username.data),
        email=bleach.clean(register_form.email.data),
...
def settings():
  ...
  if settings_form.validate_on_submit() and check_password_hash(
    ...
    user.description = bleach.clean(settings_form.description.data)
...
def Main():
  ...
  checker = threading.Thread(target=validate_online, daemon=True)
  checker.start()
  print('Serving app...')
